...
class FakeGeminiApiError(Exception):
    ...
    def __init__(self, status_code: int | None = None, message: str = ""):
        super().__init__(message or f"fake Gemini error {status_code}")
        self.status_code = status_code


def test_extract_gemini_status_code_from_status_code_attribute() -> None:
    """Verify that status_code is extracted when the SDK exposes it directly."""

    error = FakeGeminiApiError(status_code=503)

    assert _extract_gemini_status_code(error) == 503


def test_extract_gemini_status_code_from_error_text() -> None:
    """Verify that status code is extracted from Gemini error text."""
...
def test_generate_content_retries_transient_error_from_text(monkeypatch) -> None:
    """Verify that text-only 503 Gemini errors are retried."""
